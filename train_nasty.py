...
from resnet_self_student import *
...
def train_epoch_kd_adv(model, model_ad, optim, data_loader, epoch, params):
    ...
    with tqdm(total=len(data_loader)) as t:  # Use tqdm for progress bar
        for i, (train_batch, labels_batch) in enumerate(data_loader):
            ...
            output_tch, _ = model(train_batch)  # logit without SoftMax

            # teacher loss: CE(output_tch, label)
            tch_loss = nn.CrossEntropyLoss()(output_tch[0], labels_batch)

            # ############ adversarial loss ####################################
            # computer adversarial model output
            with torch.no_grad():
                output_stu, _ = model_ad(train_batch)  # logit without SoftMax
            # output_stu = output_stu.detach()

            # adversarial loss: KLdiv(output_stu, output_tch)
            T = params.temperature
            adv_loss0 = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_stu[0].detach() / T, dim=1),
                                                            F.softmax(output_tch[0] / T, dim=1)) * (T * T)
            adv_loss1 = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_stu[1].detach() / T, dim=1),
                                                            F.softmax(output_tch[1] / T, dim=1)) * (T * T)
            adv_loss2 = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_stu[2].detach() / T, dim=1),
                                                            F.softmax(output_tch[2] / T, dim=1)) * (T * T)
            adv_loss3 = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_stu[3].detach() / T, dim=1),
                                                            F.softmax(output_tch[3] / T, dim=1)) * (T * T)
            # wish to max this item

            # total loss: make the loss positive by adding a constant
            # adv_loss = adv_loss0 + adv_loss1 + adv_loss2 + adv_loss3
            adv_loss = adv_loss0
            loss = tch_loss - params.weight * adv_loss + 100.0
...
if __name__ == "__main__":
    ...
    if params.model_name == 'resnet18':
        ...
        model = resnet50()  # Change num_class inside
    ...
    if params.adversarial_model == 'resnet18':
        ...
        adversarial_model = resnet50()  # Change num_class inside
    ...
    adversarial_model.load_state_dict(checkpoint, strict=False)
